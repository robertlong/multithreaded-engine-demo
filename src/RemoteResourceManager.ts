...
export interface RemoteResourceLoader<RemoteResource = undefined> {
  type: string;
  load?(resourceId: number, resourceDef: ResourceDefinition);
  loaded?(resourceId: number, remoteResource: RemoteResource);
  loadError?(resourceId: number, error: any);
  addRef?(resourceId: number);
  removeRef?(resourceId: number);
  dispose?(resourceId: number);
}
